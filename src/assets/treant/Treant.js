...
;(function(){
	...
	*/
	var TreeNode = function (nodeStructure, id, parentId, tree, stackParentId) {
		...
		this.member_id = nodeStructure['id'];
		this.color = nodeStructure['color'];
		...
		if (!this.pseudo) {
			if (!this.nodeInnerHTML) {
				...
				if(this.image) {
					...
					image.classList.add('addMember');
					image.id = this.member_id;
					image.src = this.image;
					image.style= "border: 4px solid "+this.color
